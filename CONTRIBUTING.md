# Guía para desarrolladores — SENDA VIDA

Esta documentación es solo para quien colabora en el código o ejecuta el proyecto en local. Los usuarios finales solo necesitan descargar la APK (ver [README.md](README.md)).

## Requisitos previos

- **Java 17** (backend)
- **Node.js 18+** y **npm** (móvil)
- **PostgreSQL** (puerto 5433 por defecto en desarrollo local)
...
En desarrollo local el backend suele usar por defecto:
...
(Ajusta según tu `application.properties` o variables de entorno.)
...
En local la API queda en **http://localhost:8084/api**.

Verifica que funciona: abre http://localhost:8084/api/health en el navegador (debe devolver `{"ok":true}`).

La API en producción está en **https://senda-vida.onrender.com/api** (health: https://senda-vida.onrender.com/api/health).
...
|----------|-------------|
| `EXPO_PUBLIC_API_BASE_URL` | URL del backend. Contra producción: `https://senda-vida.onrender.com/api`. En celular físico contra tu PC: IP en la misma Wi‑Fi (ej. `http://192.168.1.100:8084/api`). En emulador Android: `http://10.0.2.2:8084/api` |
...
El script instala dependencias y lanza el build en EAS Cloud. Cuando termine, descarga la APK desde [expo.dev](https://expo.dev) o publícala en GitHub Releases. La app debe apuntar a `https://senda-vida.onrender.com/api`.

**Importante:** Ejecuta siempre desde `MOVIL` (donde están `app.json` y `package.json`). Si ejecutas `eas build` desde la raíz del repo, el build puede fallar.
